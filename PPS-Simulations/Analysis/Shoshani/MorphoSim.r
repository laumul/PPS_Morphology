...
main <- function(){
  ...
  dir=paste0(args[5],"/",args[3],"/",args[2],"/","output_",args[3])
  dir.create(paste0(dir,"/pps_Nov_post_sims"))
...
#  dat <- t(data.frame(ape::read.nexus.data(paste0(args[4],"/",args[1]))))
dat <- t(data.frame(ape::read.nexus.data("data/Shoshani_etal_2006a_paleobiodb.nex")))
#  nstate <- as.numeric(args[6])
#  states <- nstate - 1
...
if (char[1] == length(v)){ 
 ...
 nstate=2 
    ...
    tree <- ape::read.tree(paste0(args[5],"/",args[3],"/",args[2],"/output_",args[3],"/pps_Nov_posterior.var"))
    tree <- ape::root.multiPhylo (tree, tree[[1]]$tip.label[4], resolve.root = TRUE)

    
    
    InferenceLog <- read.table(paste0(args[5],"/",args[3],"/",args[2],"/output_",args[3],"/pps_Nov_posterior.var"), header=TRUE) 
    
    
    if (args[3] == "mk"){
      
      for (i in 1:500){
        dir.create(paste0(dir,"/pps_Nov_post_sims/posterior_predictive_sim_", i ))
        #alpha_morpho <- 1/InferenceLog$alpha_inv[i] 
        t <- sim.morpho(tree[[i]], k=nstate, nchar = length(v), partitions = 1, variable.coding = FALSE,
                        alpha = NULL)
        
        phangorn::write.phyDat(t, paste0(dir,"/pps_Nov_post_sims/posterior_predictive_sim_", i,"/seq.nex"), format = "nexus" )
  ...
  numbers=numbers+1
   ...
   t <- sim.morpho(tree[[i]], k = numbers, nchar = char[s], partitions = 1, variable.coding = TRUE,
                      alpha = NULL)
      phangorn::write.phyDat(t, paste0(dir,"/pps_Nov_post_sims/posterior_predictive_sim_", i, "/", "m_morph[",m,"].nex"), format = "nexus") 
      numbers=numbers+1
	m=m+1
     ...
     phangorn::write.phyDat(Morpho_seq, paste0(dir,"/pps_Nov_post_sims/posterior_predictive_sim_", i,"/seq.nex"),format ="nexus" )
