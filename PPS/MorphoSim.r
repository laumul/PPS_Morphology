...
main <- function(){
  ...
  dir=paste0("output/", args[2], "/output")
...
if (char[1] == length(v)){ 
 ...
 nstate=2 
    ...
    tree <- ape::read.tree(paste0(dir, "/APW-example.var"))
    tree <- ape::root.multiPhylo (tree, tree[[1]]$tip.label[4], resolve.root = TRUE)

    
    
    InferenceLog <- read.table(paste0(dir, "/APW-example.var"), header=TRUE) 
  ...
  numbers=numbers+1
   ...
   t <- sim.morpho(tree[[i]], k = numbers, nchar = char[s], partitions = 1, variable.coding = TRUE,
    ...
    } else if (args[2] == "mk+GVmultistate"){
